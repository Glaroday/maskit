...
3. **支持功能**：
   - **双引擎拦截**：同时覆盖标准 Fetch 与底层 XMLHttpRequest（XHR），**ChatGPT / Claude / DeepSeek 三站已实测开箱即用**，另有 14 个常见站点可在设置中一键授权添加；
   - **附件脱敏（ChatGPT / Claude 已适配）**：网页上传 Word（`.docx` / `.doc`）、Excel（`.xlsx` / `.xls`）、PPT（`.pptx`）等文件时，在本地自动脱敏后再出网；**DeepSeek 等未适配站点上传的文件本体不脱敏**，扩展会弹窗提示；
   - **回复流式还原**：模型回复在网页界面逐字流式还原，支持多轮会话一致性。
...
**不会！** Maskit 采用微秒级 SSE 事件流直通技术，上游输出一个字，本地立刻透传一个字（零整包缓冲）。首字延迟（TTFT）增量在毫秒级以内，完全保留原生打字机丝滑手感。
