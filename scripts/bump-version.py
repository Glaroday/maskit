...
用法：
  ...
  python scripts/bump-version.py 0.2.4-beta.1   # 指定预发布版本号

版本正则一律放行 prerelease 后缀（X.Y.Z-beta.N）：校验过严会让 prerelease 直接进不来，
替换正则过严则「当前就是 prerelease」时一处也替不掉（静默不动比报错更难查）。
"""
import json
import re
import subprocess
...
# 与 build.ps1 的 $verRe / check-version.py 的 SEMVER_RE 同口径
VER_RE = r"\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?"
...
def get_current_version() -> str:
    ...
    m = re.search(r"__version__\s*=\s*['\"](" + VER_RE + r")['\"]", content)
    if not m:
...
def bump_patch(ver: str) -> str:
    parts = [int(p) for p in ver.split("-")[0].split(".")]
    parts[2] += 1
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if not re.fullmatch(VER_RE, target):
    ...
    # 1. engine/panel.py（当前值可能是 prerelease，所以替换正则同样要放行后缀）
    panel_src = PANEL_PY.read_text(encoding="utf-8")
    panel_src = re.sub(r"__version__\s*=\s*['\"]" + VER_RE + r"['\"]", f"__version__ = '{target}'", panel_src, count=1)
    PANEL_PY.write_text(panel_src, encoding="utf-8")

    # 2. src-tauri/tauri.conf.json
    conf_src = TAURI_CONF.read_text(encoding="utf-8")
    conf_src = re.sub(r'"version":\s*"' + VER_RE + r'"', f'"version": "{target}"', conf_src, count=1)
    ...
    cargo_src = re.sub(r'(?m)^version\s*=\s*"' + VER_RE + r'"', f'version = "{target}"', cargo_src, count=1)
